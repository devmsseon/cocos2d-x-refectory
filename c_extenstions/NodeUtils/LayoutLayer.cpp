...
bool LayoutLayer::initLayoutLayer()
{
    ...
    this->Layer::setAnchorPoint(CCPointZero);
...
{
    if (m_frozen || !this->Layer::isVisible())
    {
        return this;
    }
...
LayoutLayer *LayoutLayer::setBackground(Node *newBackground)
{
    ...
    if (m_background)
    {
        ...
        m_background->setContentSize(this->Layer::getContentSize());
    }
...
void LayoutLayer::addChild(Node *child)
{
    //CCLayer::addChild(child);
    this->addChild(child,0,0);
}

void LayoutLayer::addChild(Node *child, int zOrder)
{
    //CCLayer::addChild(child);
    this->addChild(child, zOrder);
}

void LayoutLayer::addChild(Node *child, int zOrder, int tag)
{
...
void LayoutLayer::removeChild(Node *child, bool cleanup)
{
...
void LayoutLayer::fitChildrenSizes(bool fitWidth, bool fitHeight)
{
         ...
         ++nodeIter)
    {
        ...
        if (child && child->Layer::isVisible())
        {
            if (fitWidth)
            {
...
void HLayoutBehavior::doLayout(LayoutLayer *layer)
{
    ...
    const Node *background    = layer->getBackground();
    ...
    CCSize contentSize          = layer->Layer::getContentSize();
